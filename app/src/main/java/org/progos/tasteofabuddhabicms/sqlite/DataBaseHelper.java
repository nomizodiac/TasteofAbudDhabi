package org.progos.tasteofabuddhabicms.sqlite;
...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    private static String KEY_SCHEDULE_SLUG = "ScheduleSlug";
    ...
    private static String KEY_SCHEDULE_ITEM_TIME = "Time";
    private static String KEY_SCHEDULE_ITEM_ACT = "Act";
    ...
    public int addSchedules(ArrayList<Schedule> schedules) {
        ...
        for (int i = 0; i < schedules.size(); i++) {
            ...
            values.put(KEY_SCHEDULE_SLUG, schedules.get(i).getSlug());
    ...
    public ArrayList<Schedule> getSchedules() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                String keyScheduleSlug = cursor.isNull(cursor.getColumnIndex(KEY_SCHEDULE_SLUG)) ? "" : cursor.getString(cursor.getColumnIndex(KEY_SCHEDULE_SLUG));
                ...
                Schedule schedule = new Schedule(keyScheduleId, keyScheduleSlug, keyScheduleDay, keyScheduleDate);
    ...
    public int addScheduleItems(ArrayList<ScheduleItem> scheduleItems) {

        int noOfRowsInserted = 0;
        SQLiteDatabase db = this.getReadableDatabase();
        for (int i = 0; i < scheduleItems.size(); i++) {
            ContentValues values = new ContentValues();
            values.put(KEY_SCHEDULE_ITEM_TIME, scheduleItems.get(i).getTime());
            values.put(KEY_SCHEDULE_ITEM_ACT, scheduleItems.get(i).getAct());
            values.put(KEY_SCHEDULE_ID, scheduleItems.get(i).getScheduleId());

            int messageCount = getRecordCountScheduleItem(scheduleItems.get(i).getTime(), scheduleItems.get(i).getAct(), scheduleItems.get(i).getScheduleId());
            if (messageCount == 0) {
                long idRowInserted = db.insert(TABLE_SCHEDULE_ITEMS, null, values);
                if (idRowInserted != -1)
                    noOfRowsInserted++;
            } else {
                db.update(TABLE_SCHEDULE_ITEMS, values, KEY_SCHEDULE_ITEM_TIME + " = ? AND " + KEY_SCHEDULE_ITEM_ACT + " = ? AND " + KEY_SCHEDULE_ID + " = ?", new String[]{scheduleItems.get(i).getTime(), scheduleItems.get(i).getAct(), scheduleItems.get(i).getScheduleId()});
            }
        }
        db.close();
        return noOfRowsInserted;
    ...
    public ArrayList<ScheduleItem> getScheduleItems(Schedule schedule) {
        SQLiteDatabase db = this.getWritableDatabase();
        ArrayList<ScheduleItem> scheduleItems = new ArrayList<>();
        Cursor cursor = db.query(TABLE_SCHEDULE_ITEMS, null, KEY_SCHEDULE_ID + " = ?", new String[]{schedule.getId()}, null, null, null);
        if (cursor.moveToFirst()) {
            do {
                String keyScheduleItemTime = cursor.isNull(cursor.getColumnIndex(KEY_SCHEDULE_ITEM_TIME)) ? "" : cursor.getString(cursor.getColumnIndex(KEY_SCHEDULE_ITEM_TIME));
                String keyScheduleItemAct = cursor.isNull(cursor.getColumnIndex(KEY_SCHEDULE_ITEM_ACT)) ? "" : cursor.getString(cursor.getColumnIndex(KEY_SCHEDULE_ITEM_ACT));
                String keyScheduleId = cursor.isNull(cursor.getColumnIndex(KEY_SCHEDULE_ID)) ? "" : cursor.getString(cursor.getColumnIndex(KEY_SCHEDULE_ID));

                ScheduleItem scheduleItem = new ScheduleItem(keyScheduleItemTime, keyScheduleItemAct, keyScheduleId);
    ...
    private int getRecordCountScheduleItem(String itemTime, String itemAct, String scheduleId) {
        int nCount = 0;
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.query(TABLE_SCHEDULE_ITEMS, null, KEY_SCHEDULE_ITEM_TIME + " = ? AND " + KEY_SCHEDULE_ITEM_ACT + " = ? AND " + KEY_SCHEDULE_ID + " = ?", new String[]{itemTime, itemAct, scheduleId}, null, null, null);
        nCount = cursor.getCount();
        cursor.close();
        return nCount;
    }
